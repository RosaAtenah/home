...
void putLog(char * login,char * passwd){
	FILE * fic=fopen("/home/mit/CGI/sign.txt","a+");
	if(fic == NULL) {
...
void form(){
	...
	printf("<head><meta charset=\"UTF-8\"><title>Log In</title>"
			...
			"</head>"
			"<body>"
			"<form action=""http://www.connex.cgi"" method=""get"">"
			"<div>"
				"<h3>complete your information here to create a count</h3>"
				"<a href=""http://www.sign.mit"">sign in</a>"
				"<a href=""http://www.login.mit"">log in</a>"
				"<label for="">Login</label>"
				"<input type=""text"" name=""login"" required>"
				"<label for="""">Password</label>"
				"<input type=""password"" name=""password"" id=""pwd"" pattern=""{5,}"" required>"
				"<input type=""submit"" value=""Send"" class=""submit"">"
				"<p><a href='http://www.passwd.mit'>Click here if you forgot your password</a></p>"
			"</div>"
			"</form>"
			"</body>"
			"</html>"
